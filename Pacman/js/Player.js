...
function wrapTunnel(x, y) {
  ...
  if (!inBounds(Math.floor(nx), Math.floor(ny))) {
    console.warn("[Player] posición tras túnel fuera de rango, se corrige.");
    nx = clamp(nx, 0.5, cols() - 0.5);
    ny = clamp(ny, 0.5, rows() - 0.5);
  }
  return { x: nx, y: ny };
}
